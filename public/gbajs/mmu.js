MemoryView = function MemoryView(memory, offset) {
	this.inherit();
	this.buffer = memory;
...
MemoryBlock = function MemoryBlock(size, cacheBits) {
...
ROMView = function ROMView(rom, offset) {
...
BIOSView = function BIOSView(rom, offset) {
	MemoryView.call(this, rom, offset);
...
BadMemory = function BadMemory(mmu, cpu) {
	this.inherit();
	this.cpu = cpu;
	this.mmu = mmu;
};
...
GameBoyAdvanceMMU = function GameBoyAdvanceMMU() {
	this.inherit();
...
GameBoyAdvanceMMU.prototype.memoryMatches = function (offset, values) {
	for (let i = 0; i < values.length; ++i) {
		const memoryValue = this.loadU8(offset + i);

		if (memoryValue !== values[i]) {
			return false;
		}
	}
...
https: GameBoyAdvanceMMU.prototype.loadSavedata = function (save) {
...
GameBoyAdvanceMMU.prototype.saveNeedsFlush = function () {
	return !!(this.save && this.save.writePending);
};

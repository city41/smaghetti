...
const JSON_DEST_DIR = 'public/level-archive/';
const PAGE_SIZE = 20;

async function writeTotalCountMapSrc(totalCountMap) {
	const destPath = 'src/components/levels/_totalCountMap.ts';

	const totalCountMapSrc = `
// generated by ${__filename}
export const TOTAL_COUNT_MAP: Record<string, number> = ${JSON.stringify(
		totalCountMap
	)};
`;

	await fsp.writeFile(destPath, totalCountMapSrc);
...
async function getMainPublishedLevels() {
	...
	for (const fetchUrl of FETCH_URLS) {
		for (const order of ORDERS) {
			for (const tag of fetchUrl.tags) {
				...
				if (fetchUrl.addTotalCountToMap) {
					totalCountMap[tag || fetchUrl.fileRoot] = levelCount;
				}
			}
		}
	}
	...
	console.log(
		'\n\ntotalCountMap',
		JSON.stringify(totalCountMap, null, 2),
		'\n\n'
	);

	await writeTotalCountMapSrc(totalCountMap);
...
async function getCreatorPages(creators) {
	...
	for (const creator of creators) {
		...
		const destPath = path.resolve(process.cwd(), JSON_DEST_DIR, fileName);
		await fsp.writeFile(destPath, data);
		console.log('wrote', destPath);
	}
}

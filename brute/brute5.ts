...
function capture(
	...
	return new Promise((resolve) => {
		return getGbaScreen(objectSet, graphicSet, [0x42, 22, 5, id, 3])
			.then(({ buffer, data }) => {
...
objectSet = ROOM_TYPE_SETTINGS.fortress.objectSet;
let graphicSet = ROOM_TYPE_SETTINGS.fortress.objectGraphicSet;
...
function onCaptureSet() {
	...
	if (false && objectSet < (offset + 1) * chunkSize && objectSet < 0x10) {

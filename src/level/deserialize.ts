...
function getCorner(entity: Entity): GoalStartCorner {
  if (entity.x === 0) {
    if (entity.y < TILE_SIZE * 5) {
      return 'upper-left';
    }
    return 'lower-left';
  }
...
function deserialize(
  ...
  const deserializedLevelData = {
    ...
    startCorner: getCorner(normalizedEntities.find(e => e.type === 'Start')!),
    goalCorner: getCorner(normalizedEntities.find(e => e.type === 'Goal')!),
  };

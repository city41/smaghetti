...
const editorSlice = createSlice({
  ...
  reducers: {
    ...
    toggleGhosts(state: InternalEditorState) {
    ...
    ghostsCollected(
      state: InternalEditorState,
      action: PayloadAction<Ghost[]>
    ) {
      if (action.payload.length > 1) {
        state.ghosts = uniqBy(action.payload, (g) => `${g.x}-${g.y}-${g.tile}`);
      }
    },
...
const undoableReducer = undoable(cleanUpReducer, {
  filter: excludeAction([
    ...
    pan.toString(),
    ghostsCollected.toString(),
    toggleGhosts.toString(),
    toggleGrid.toString(),
